...
const (
	...
	VmiFedoraCoreSecure  = "vmi-fedora-core-secure"
...
const (
	...
	imageFedoraCore = "fedora-coreos-container-disk-demo"
)
...
func GetVMIFedoraCoreSecure() *v1.VirtualMachineInstance {
	vmi := getBaseVMI(VmiFedoraCoreSecure)

	addContainerDisk(&vmi.Spec, fmt.Sprintf("%s/%s:%s", DockerPrefix, imageFedoraCore, DockerTag), busVirtio)
