...
var _ = Describe("[Serial][sig-compute]MediatedDevices", func() {
	...
	Context("with generic mediated devices", func() {
		...
		const disableCmdFmt = "/usr/lib/nvidia/sriov-manage -d %s"
		const enableCmdFmt = "/usr/lib/nvidia/sriov-manage -e %s"
		const uuidRegex = "????????-????-????-????-????????????"
		...
		AfterEach(func() {
			if CurrentSpecReport().Failed() && rootPCIId != "none" {
				...
				_ = runBashCmdRw(fmt.Sprintf(enableCmdFmt, rootPCIId))
			}
		})

		It("should create mdevs on devices that appear after CR configuration", func() {
			By("DNM: inspect sysfs")
			_ = runBashCmdRw("ls -lR /sys/class/mdev_bus ; ls -lR /sys/bus/mdev ; ls -lR /sys/devices")
			...
			driverPath, e, err = runBashCmd("readlink -e " + mdevBusPath + pciId + "/driver || echo /sys/bus/pci/drivers/nvidia")
			Expect(err).ToNot(HaveOccurred(), e)
			...
			By("disabling the virtual functions of the device")
			re := regexp.MustCompile(`[\da-f]{2}\.[\da-f]'$`)
			rootPCIId = re.ReplaceAllString(pciId, "00.0'")
			err = runBashCmdRw(fmt.Sprintf(disableCmdFmt, rootPCIId))
			Expect(err).ToNot(HaveOccurred())
			...
			By("re-enabling the virtual functions of the device")
			err = runBashCmdRw(fmt.Sprintf(enableCmdFmt, rootPCIId))
			Expect(err).ToNot(HaveOccurred())
			Eventually(func() error {
				_, _, err = runBashCmd("ls " + mdevBusPath + pciId)
				return err
