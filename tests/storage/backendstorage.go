...
var _ = SIGDescribe("[Serial]Backend Storage", Serial, func() {
	...
	It("Should use the default storage class when none is specified in the CR", func() {
		By("Removing the VM state storage class from the CR")
		...
		By("Expecting the creation of a backend storage PVC with the default storage class")
		...
		storageClass, err := virtClient.StorageV1().StorageClasses().Get(context.Background(), *pvc.Spec.StorageClassName, metav1.GetOptions{})
		Expect(storageClass.Annotations).To(HaveKeyWithValue("storageclass.kubernetes.io/is-default-class", "true"))
	})
	...
	It("Should use RWO when RWX is not supported", func() {
		...
		for _, sp := range sps.Items {
			...
			for _, property := range sp.Status.ClaimPropertySets {
				if property.VolumeMode != nil && *property.VolumeMode == corev1.PersistentVolumeFilesystem {
		...
		for _, cond := range vmi.Status.Conditions {
			if cond.Type == v1.VirtualMachineInstanceIsMigratable {
				found = true
		...
		Expect(found).To(BeTrue(), "LiveMigratable condition not found")
	})
})
