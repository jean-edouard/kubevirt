...
var _ = SIGDescribe("[Serial]Backend Storage", Serial, func() {
	...
	Context("Block storage", func() {
		...
		DescribeTable("when using block storage", func(featureGate string, shouldBeMigratable bool) {
			var storageClass string
			...
			if kv.Spec.Configuration.DeveloperConfiguration == nil {
				kv.Spec.Configuration.DeveloperConfiguration = pointer.P(v1.DeveloperConfiguration{})
			}
			...
			if featureGate != "" {
				kv.Spec.Configuration.DeveloperConfiguration.FeatureGates = append(kv.Spec.Configuration.DeveloperConfiguration.FeatureGates, featureGate)
			}
			tests.UpdateKubeVirtConfigValueAndWait(kv.Spec.Configuration)
			...
			Entry("with the feature gate set, the VM should be migratable", "MigrateBlockBackendStorage", true),
			Entry("without the feature gate, the VM should be non-migratable", "", false),
		)
	})
})
