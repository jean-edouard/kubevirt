...
var _ = Describe("[sig-compute]Configurations", func() {
	...
	Describe("VirtualMachineInstance definition", func() {
		...
		Context("[rfe_id:140][crit:medium][vendor:cnv-qe@redhat.com][level:component]with namespace memory limits lower than VMI required memory", func() {
			...
			It("[test_id:1670]should failed to start the VMI", func() {
				...
				limitRangeObj := kubev1.LimitRange{
					...
					ObjectMeta: metav1.ObjectMeta{Name: "abc1", Namespace: testsuite.GetTestNamespace(vmi)},
				...
				_, err := virtClient.CoreV1().LimitRanges(testsuite.GetTestNamespace(vmi)).Create(context.Background(), &limitRangeObj, metav1.CreateOptions{})
				Expect(err).ToNot(HaveOccurred())
		...
		Context("[rfe_id:140][crit:medium][vendor:cnv-qe@redhat.com][level:component]with namespace limits higher than VMI requests", func() {
			...
			It("[test_id:3120]should start the VMI with the right default settings from namespace limits", func() {
				...
				Eventually(func() bool {
					...
					err = virtClient.VirtualMachineInstance(testsuite.GetTestNamespace(vmi)).Delete(createdVMI.Name, &metav1.DeleteOptions{})
		...
		Context("[rfe_id:140][crit:medium][vendor:cnv-qe@redhat.com][level:component]with hugepages", func() {
			...
			DescribeTable("should consume hugepages ", func(hugepageSize string, memory string, guestMemory string, option1, option2 libvmi.Option, namespace string) {
				...
				Entry("[Serial][test_id:1671]hugepages-2Mi", Serial, "2Mi", "64Mi", "None", nil, nil, testsuite.GetTestNamespace(nil)),
				Entry("[Serial][test_id:1672]hugepages-1Gi", Serial, "1Gi", "1Gi", "None", nil, nil, testsuite.GetTestNamespace(nil)),
				Entry("[Serial][test_id:1672]hugepages-2Mi with guest memory set explicitly", Serial, "2Mi", "70Mi", "64Mi", nil, nil, testsuite.GetTestNamespace(nil)),
	...
	Context("with a custom scheduler", func() {
		It("[test_id:4631]should set the custom scheduler on the pod", func() {
			...
			launcherPod := libvmi.GetPodByVirtualMachineInstance(runningVMI, testsuite.GetTestNamespace(vmi))
	...
	Context("[rfe_id:140][crit:medium][vendor:cnv-qe@redhat.com][level:component]with CPU request settings", func() {
		...
		It("[test_id:3127]should set CPU request from VMI spec", func() {
			...
			readyPod := libvmi.GetPodByVirtualMachineInstance(runningVMI, testsuite.GetTestNamespace(vmi))
