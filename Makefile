...
all:
	hack/dockerized "export BUILD_ARCH=${BUILD_ARCH} && DOCKER_PREFIX=${DOCKER_PREFIX} DOCKER_TAG=${DOCKER_TAG} IMAGE_PULL_POLICY=${IMAGE_PULL_POLICY} VIRT_REPLICAS=${VIRT_REPLICAS} VERBOSITY=${VERBOSITY} ./hack/build-manifests.sh && \
	    hack/bazel-fmt.sh && hack/bazel-build.sh"

go-all:
	hack/dockerized "export KUBEVIRT_NO_BAZEL=true && KUBEVIRT_VERSION=${KUBEVIRT_VERSION} ./hack/build-go.sh install ${WHAT} && ./hack/build-copy-artifacts.sh ${WHAT} && DOCKER_PREFIX=${DOCKER_PREFIX} DOCKER_TAG=${DOCKER_TAG} IMAGE_PULL_POLICY=${IMAGE_PULL_POLICY} VIRT_REPLICAS=${VIRT_REPLICAS} VERBOSITY=${VERBOSITY} GO_BUILD=true ./hack/build-manifests.sh"
...
manifests:
	hack/dockerized "CSV_VERSION=${CSV_VERSION} QUAY_REPOSITORY=${QUAY_REPOSITORY} \
	  ...
	  IMAGE_PULL_POLICY=${IMAGE_PULL_POLICY} VIRT_REPLICAS=${VIRT_REPLICAS} VERBOSITY=${VERBOSITY} PACKAGE_NAME=${PACKAGE_NAME} \
