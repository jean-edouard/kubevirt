...
var _ = Describe("Validating VM Admitter", func() {
	...
	enableLiveUpdate := func() {
		...
		if kv.Spec.Configuration.DeveloperConfiguration.FeatureGates == nil {
			kv.Spec.Configuration.DeveloperConfiguration.FeatureGates = []string{virtconfig.VMLiveUpdateFeaturesGate}
		} else {
			kv.Spec.Configuration.DeveloperConfiguration.FeatureGates = append(kv.Spec.Configuration.DeveloperConfiguration.FeatureGates, virtconfig.VMLiveUpdateFeaturesGate)
		}
	...
	disableLiveUpdate := func() {
		kv := testutils.GetFakeKubeVirtClusterConfig(kvInformer)
		// Removing all feature gates, this is only called at the end of tests anyway
		if kv.Spec.Configuration.DeveloperConfiguration != nil {
			kv.Spec.Configuration.DeveloperConfiguration.FeatureGates = make([]string, 0)
		}
