...
func (l *LibvirtDomainManager) setGuestTime(vmi *v1.VirtualMachineInstance) {
	...
	//
	// Each call cancels any in-flight goroutine via getGuestTimeContext()
	// and starts a new one with a fresh retry window, so a later unpause
	// can still sync after a previous attempt timed out or was cancelled.
