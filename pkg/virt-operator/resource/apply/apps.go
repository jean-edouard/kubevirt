...
func (r *Reconciler) syncPodDisruptionBudgetForDeployment(deployment *appsv1.Deployment) error {
	...
	if podDisruptionBudget.Spec.MinAvailable.IntVal == 0 {
		var err error
		if exists {
	...
	if !*modified &&
		existingCopy.Spec.MinAvailable.IntVal == podDisruptionBudget.Spec.MinAvailable.IntVal &&
	...
	SetGeneration(&kv.Status.Generations, podDisruptionBudget)
	log.Log.V(2).Infof("poddisruptionbudget %v patched with: %s", podDisruptionBudget.GetName(), string(generatePatchBytes(ops)))
