...
func (k *KubeVirtTestData) addAll(config *util.KubeVirtDeploymentConfig, kv *v1.KubeVirt) {
	...
	apiDeployment, _ := components.NewApiServerDeployment(NAMESPACE, config.GetImageRegistry(), config.GetImagePrefix(), config.GetApiVersion(), "", "", 2, config.GetImagePullPolicy(), config.GetVerbosity(), config.GetExtraEnv())
	apiDeploymentPdb := components.NewPodDisruptionBudgetForDeployment(apiDeployment)
	controller, _ := components.NewControllerDeployment(NAMESPACE, config.GetImageRegistry(), config.GetImagePrefix(), config.GetControllerVersion(), config.GetLauncherVersion(), "", "", 2, config.GetImagePullPolicy(), config.GetVerbosity(), config.GetExtraEnv())
...
func (k *KubeVirtTestData) addPodsWithIndividualConfigs(config *util.KubeVirtDeploymentConfig,
	...
	controller, _ := components.NewControllerDeployment(NAMESPACE, configController.GetImageRegistry(), configController.GetImagePrefix(), configController.GetControllerVersion(), configController.GetLauncherVersion(), "", "", 2, configController.GetImagePullPolicy(), configController.GetVerbosity(), configController.GetExtraEnv())
	pod = &k8sv1.Pod{
...
var _ = Describe("KubeVirt Operator", func() {
	...
	Context("On valid KubeVirt object", func() {
		...
		It("should create an api server deployment with passthrough env vars, if provided in config", func(done Done) {
			...
			apiDeployment, err := components.NewApiServerDeployment(NAMESPACE, config.GetImageRegistry(), config.GetImagePrefix(), config.GetApiVersion(), "", "", 2, config.GetImagePullPolicy(), config.GetVerbosity(), config.GetExtraEnv())
		...
		It("should create a controller deployment with passthrough env vars, if provided in config", func(done Done) {
			...
			controllerDeployment, err := components.NewControllerDeployment(NAMESPACE, config.GetImageRegistry(), config.GetImagePrefix(), config.GetControllerVersion(), config.GetLauncherVersion(), "", "", 2, config.GetImagePullPolicy(), config.GetVerbosity(), config.GetExtraEnv())
