...
func (k *KvmVirtRuntime) configureVhostCgroup(vmi *v1.VirtualMachineInstance, cgroupManager cgroup.Manager, domain *api.Domain) error {
	...
	vhostPrefix := "vhost-" + strconv.Itoa(qemupid)
	...
	for _, proc := range processes {
		if proc.Executable() != vhostPrefix {
			continue
		}
		tid := proc.Pid()
		k.logger.V(3).Object(vmi).Infof("pinning vhost thread %d (%s) to cpus %v", tid, proc.Executable(), vhostCPUs)
