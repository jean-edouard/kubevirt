...
func (d *VirtualMachineController) migrationTargetUpdateVMIStatus(vmi *v1.VirtualMachineInstance, domainExists bool) error {
	...
	if !migrations.IsMigrating(vmi) {
		...
		if hostAddress != migrationIpAddress {
			...
			d.recorder.Event(vmi, k8sv1.EventTypeNormal, v1.PreparingTarget.String(), fmt.Sprintf("Migration Target is listening at %s INSTEAD OF %s, on ports: %s", migrationIpAddress, d.ipAddress, portsStrList))
